...
watch(
  ...
  {
    immediate: true, //初始化立即執行
  }
);

const contentMenuVisible = ref(false); // 右鍵菜單是否顯示
...
function moveToCurrentTag() {
  // 使用 nextTick() 的目的是確保在更新 tagsView 組件之前，scrollPaneRef 對象已經滾動到了正確的位置。
  nextTick(() => {
...
// 如果是混合模式，更改selectedTag，需要對應高亮的activeTop
watch(
  () => route.name,
  (newVal) => {
    if (newVal) {
