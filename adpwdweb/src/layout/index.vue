...
const WIDTH_DESKTOP = 992; // 響應式佈局容器固定寬度  大屏（>=1200px） 中屏（>=992px） 小屏（>=768px）
const isMobile = computed(() => appStore.device === DeviceEnum.MOBILE);
const isOpenSidebar = computed(() => appStore.sidebar.opened);
const showTagsView = computed(() => settingsStore.tagsView); // 是否顯示tagsView
const layout = computed(() => settingsStore.layout); // 佈局模式 left top mix
const activeTopMenuPath = computed(() => appStore.activeTopMenuPath); // 頂部菜單激活path
const mixLeftMenus = computed(() => permissionStore.mixLeftMenus); // 混合佈局左側菜單
