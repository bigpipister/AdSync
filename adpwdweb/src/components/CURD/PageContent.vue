          ...
          <template v-if="typeof item === 'string'">
            ...
            <template v-else-if="item === 'filter'">
              <el-popover placement="bottom" trigger="click">
                <template #reference>
                  <el-button icon="Operation" circle title="篩選列" />
                </template>
        ...
        <el-table-column v-if="col.show" v-bind="col">
          <template #default="scope">
            ...
            <template v-else-if="col.templet === 'switch'">
              <template v-if="col.prop">
                <!-- pageData.length>0: 解決el-switch組件會在表格初始化的時候觸發一次change事件 -->
                <el-switch
    ...
    >
      ...
      <el-scrollbar max-height="60vh">
        ...
        >
          ...
          <el-form-item label="數據源" prop="origin">
            <el-select v-model="exportsFormData.origin">
              <el-option label="當前數據 (當前頁的數據)" :value="ExportsOriginEnum.CURRENT" />
              <el-option
                label="選中數據 (所有選中的數據)"
...
const exportsFormRules: FormRules = {
  fields: [{ required: true, message: "請選擇字段" }],
  origin: [{ required: true, message: "請選擇數據源" }],
};
// 打開導出彈窗
...
const importFormRules: FormRules = {
  files: [{ required: true, message: "請選擇文件" }],
};
// 打開導入彈窗
